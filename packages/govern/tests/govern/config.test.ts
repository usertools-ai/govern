...
function makeTmpDir(): string {
	const dir = join(tmpdir(), `govern-config-test-${randomUUID()}`);
...
describe("loadConfig()", () => {
	...
	it("loads config from existing govern.config.json", async () => {
	...
	it("handles malformed JSON gracefully (throws)", async () => {
		const vaultDir = join(tmpDir, VAULT_DIR);
		mkdirSync(vaultDir, { recursive: true });
		writeFileSync(join(vaultDir, "govern.config.json"), "{ not valid json }");
