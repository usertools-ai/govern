...
describe("govern init", () => {
	...
	beforeEach(() => {
		tempDir = mkdtempSync(join(tmpdir(), "govern-init-"));
	...
	it("creates the .usertools directory structure", async () => {
		await run(tempDir);
	...
	it("writes default govern.config.json", async () => {
		await run(tempDir);

		const configPath = join(tempDir, ".usertools", "govern.config.json");
	...
	it("writes default policies/default.yml", async () => {
		...
		const policyPath = join(tempDir, ".usertools", "policies", "default.yml");
	...
	it("writes .gitignore", async () => {
		...
		const gitignorePath = join(tempDir, ".usertools", ".gitignore");
